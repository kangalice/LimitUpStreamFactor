...
#ifndef EVENTDRIVENSTRATEGY_SSS_AFTER_PBTIMEMAX_8_H
#define EVENTDRIVENSTRATEGY_SSS_AFTER_PBTIMEMAX_8_H
...
namespace vendor {
    namespace strategy {
        namespace eventdriven {
            class Sss_after_pbtimemax_8 : public BaseFactor {
            private:
                int zcz;
                double linePrice;
                bool isAbove = false;
                ...
                int64_t maxMs = 0;
            public:
                Sss_after_pbtimemax_8(MarketDataManager *manager,
                                         std::unordered_map<std::string, double> *factor_map)
                        : BaseFactor(
                        manager, factor_map) {
                    this->factor_names = std::vector<std::string>{"sss_after_pbtimemax_8"};
...
#endif //EVENTDRIVENSTRATEGY_SSS_AFTER_PBTIMEMAX_8_H
